...
with video_tab:
    ...
    if uploaded_video is None:
        ...
        try:
            ...
            m2.metric("👥 检测总人数", total_persons)
            m3.metric("⚠️ 未佩戴头盔", no_helmet_count)
            m4.metric("📈 头盔佩戴率", f"{helmet_rate:.1f}%")

            violations = stats.get("frames_with_violations", {})
            if violations:
                st.warning(f"⚠️ 检测到 **{len(violations)}** 个违规帧。")
                with st.expander("查看违规帧明细"):
                    rows = [
                        {"帧号": frame_idx, "未佩戴头盔人数": count}
                        for frame_idx, count in sorted(violations.items())
                    ]
                    st.dataframe(rows, use_container_width=True)
            else:
                st.success("✅ 未发现未佩戴头盔人员。")
