...
> 项目整理说明：根目录为唯一实现来源；`detector/` 已统一仅保留一份（位于根目录），`helmet-detection-app/` 仅保留历史启动兼容入口。
...
- ✅ **实时检测标注** - 在图片上绘制检测框和标签
- ✅ **统计分析** - 逐张显示及批量汇总骑行人员/乘员未佩戴头盔人数和佩戴率
- ✅ **结果导出** - 支持逐张下载标注结果图片
...
> 说明：当前模型只会标注**骑电动车的人员/乘员**是否佩戴安全帽；未骑电动车的普通行人不会被框出，也不会进入统计。
...
项目默认使用本地模型文件：
1. 将从 `huggingface://tdcdpd/Helmet_Detection` 下载得到的权重保存为 `models/best.pt`
2. 应用优先加载 `models/best.pt`，无需每次联网下载
3. 仅在本地文件不存在且你配置为仓库ID时，才会尝试 HuggingFace 回退加载
